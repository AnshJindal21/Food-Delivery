...
export default function Home() {
  ...
  const loadData = async () => {
    try {
      let response = await fetch("https://food-delivery-backend-c90u.onrender.com/api/foodData", {
        method: "POST",
        headers: {
  ...
  return (
    //Corousel cannot be alag component as search bar mai filter lagana yaha 
    <div>
      <div>
        <Navbar />
      </div>
    ...
    <span className="visually-hidden">Next</span>
      ...
      <div className="container">
        {foodCat.length > 0 ? (
          foodCat.map((data) => {
            return (
              //row and col is bootstrap grid to make our site responsive
